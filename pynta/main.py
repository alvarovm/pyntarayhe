            ...
            for adsname,adsorbate in adsorbate_dict.items():
                ...
                for prefix,structure in adsorbate.items():
                    ...
                    fwopt2 = optimize_firework(os.path.join(self.path,"Adsorbates",adsname,str(prefix),"weakopt_"+str(prefix)+".xyz"),
                        ...
                        #run_kwargs={"fmax" : self.fmaxopt, "steps" : 70},parents=[fwopt],constraints=constraints, ## original
                        run_kwargs={"fmax" : 0.0000001, "steps" : 70},parents=[fwopt],constraints=constraints,
...
#restart option: RHE
    def reset(self):
        ...
        #rhe    else:
        #rhe        task_id = int(task_name.split('--')[-1])
        #rhe        if task_id in [3, 5, 7, 9, 12, 14, 16, 18]:
        #rhe            d = self.launchpad.get_fw_dict_by_id(task_id)
        #rhe            newd = deepcopy(d['spec'])
        #rhe            from pprint import pprint
        #rhe            nameTask = newd['_tasks'][0]['_fw_name']
        #rhe            print("  TASKID ",task_id)
        #rhe            pprint(newd)

        #rhe            nameTasks = ['{{pynta.tasks.MolecularOptimizationTask}}',
        #rhe                        '{{pynta.tasks.MolecularVibrationsTask}}',
        #rhe                        '{{pynta.tasks.MolecularIRC}}']

        #rhe            print('TaskID:{2:5d} Name task {0:^20s} {1:^12s}'.format(task_name, task_state, task_id))

        #rhe            self.launchpad.rerun_fw(task_id)
        #rhe            self.launchpad.update_spec([task_id], newd)
